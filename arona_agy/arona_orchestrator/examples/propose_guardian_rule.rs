...
fn main() -> anyhow::Result<()> {
    ...
    // 注意: Guardianの守備範囲は4つの脅威カテゴリ(ランサムウェア・ハード故障・
    // データ破損・システム破損)に限定されている(設計まとめ 3〜4章)。
    // ブルートフォース攻撃のような侵入検知はこの範囲外のため、
    // 必ずいずれかのカテゴリに収まるシナリオを与える。
    let situation = "r4giさんのFiveMサーバーで、瞬断(電源の一瞬の途絶)が発生した\
        直後から、データベースへの書き込みが3回連続で失敗し、一部テーブルに\
        欠損の疑いがある。既存のGuardianルールには、この種の書き込み失敗の\
        連続発生を検知するものがまだない。";
