...
/// `cuda`フィーチャーが有効ならGPUを使い、初期化に失敗した場合や
/// フィーチャーが無効な場合はCPUにフォールバックする。
/// 黙ってフォールバックせず、必ずどちらを使っているかを表示する
/// (Ollamaで踏んだ「気づかないうちに意図しない挙動になる」問題を避けるため)。
fn select_device() -> Device {
    #[cfg(feature = "cuda")]
    {
        match Device::new_cuda(0) {
            Ok(device) => {
                eprintln!("[candle_backend] CUDA GPU(device 0)を使用します");
                return device;
            }
            Err(e) => {
                eprintln!(
                    "[candle_backend] CUDA初期化に失敗したため、CPUにフォールバックします: {e}"
                );
            }
        }
    }
    ...
    {
        eprintln!("[candle_backend] cudaフィーチャーが無効なため、CPUで動作します");
    }
    Device::Cpu
}